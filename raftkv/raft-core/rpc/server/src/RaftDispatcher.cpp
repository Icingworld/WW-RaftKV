...
RaftDispatcher::RaftDispatcher(const std::string & _Ip, const std::string & _Port)
    : _Ip(_Ip)
    , _Port(_Port)
    , _Service_map()
    , _Event_loop()
...
void RaftDispatcher::run()
{
    muduo::net::InetAddress address(_Ip, std::stoi(_Port));
    muduo::net::TcpServer server(&_Event_loop, address, "RaftDispatcher");

    // 设置回调函数
    server.setConnectionCallback(std::bind(&RaftDispatcher::_OnConnection, this, std::placeholders::_1));
...
void RaftDispatcher::_OnConnection(const muduo::net::TcpConnectionPtr & _Conn)
{
    if (!_Conn->connected()) {
        // 连接关闭，断开连接
        _Conn->shutdown();
...
void RaftDispatcher::_OnMessage(const muduo::net::TcpConnectionPtr & _Conn, muduo::net::Buffer * _Buffer, muduo::Timestamp _Receive_time)
{
    ...
    google::protobuf::Closure * done = google::protobuf::NewCallback<
        RaftDispatcher, const muduo::net::TcpConnectionPtr &, google::protobuf::Message *
    >(this, &RaftDispatcher::_SendResponse, _Conn, response.get());

    // 调用实现
    info._Service->CallMethod(method_it->second, nullptr, request.get(), response.get(), done);
}

void RaftDispatcher::_SendResponse(const muduo::net::TcpConnectionPtr & _Conn, google::protobuf::Message * _Response)
{
