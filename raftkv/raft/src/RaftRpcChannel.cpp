...
RaftRpcChannel::RaftRpcChannel(const std::string & _Ip, const std::string & _Port)
    : _Ip(_Ip)
    , _Port(_Port)
    , _Event_loop()
    , _Context()
...
void RaftRpcChannel::_OnConnection(const muduo::net::TcpConnectionPtr & _Conn)
{
    if (_Conn->connected()) {
        ...
        // 序列化请求
        std::string request_str;
        if (!RaftRpcSerialization::serialize(service_name, method_name, *_Context._Request, request_str)) {
            // 序列化失败
            _Conn->shutdown();
            _Event_loop.quit();
            return;
